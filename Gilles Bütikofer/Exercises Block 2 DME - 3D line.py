...
def plot_raster_magma_nodata_gray(src, title="Terrain raster"):
    """
    Plot raster band 1 with magma colormap.
    No-data / NaN values are shown in grey.
    """

    data = src.read(1).astype(float)
    nodata = src.nodata

    # Convert raster no-data values to NaN
    if nodata is not None:
        data[data == nodata] = np.nan

    # Mask NaN / invalid values
    data_masked = np.ma.masked_invalid(data)

    # Copy magma colormap and set masked values to grey
...
# plot_raster_magma_nodata_gray(src, title="Terrain raster - no-data in grey")
...
# Local metric coordinate system for the line-of-sight model.
# The terrain raster is still sampled in lon/lat, but the ray itself is
# constructed in a real x/y/z coordinate system: Swiss LV95 meters + altitude.
los_crs = "EPSG:2056"
to_los_crs = Transformer.from_crs("EPSG:4326", los_crs, always_xy=True)
from_los_crs = Transformer.from_crs(los_crs, "EPSG:4326", always_xy=True)
...
aircraft_signal_altitude_m = aircraft_altitude_m + aircraft_antenna_height_m
dme_signal_altitudes_m = dme_alts + dme_antenna_height_m

for i in range(len(dme_lons)):

    horizontal_dist_km = haversine_km(
        XX,
        YY,
        dme_lons[i],
        dme_lats[i]
    ...
    vertical_dist_km = (
        aircraft_signal_altitude_m - dme_signal_altitudes_m[i]
    ) / 1000

    slant_dist_km = np.sqrt(horizontal_dist_km**2 + vertical_dist_km**2)

    coverage_free += slant_dist_km <= dme_ranges[i]
...
def is_visible_los_fast(lon_aircraft, lat_aircraft, alt_aircraft,
                        ...
                        n_samples=50):
    """Return True if terrain does not block the DME-aircraft path.

    This version constructs the radio ray as a local 3-D line:
        x/y = Swiss LV95 coordinates in meters
        z   = altitude in meters above mean sea level

    The previous version interpolated directly in longitude/latitude. That is
    acceptable for small areas, but it is still a geographic 2-D path. Using a
    local metric CRS makes the line-of-sight geometry consistent with the
    altitude axis.
    """

    # Fraction along the DME-aircraft path.
    fractions = np.linspace(0, 1, n_samples)

    # Convert endpoints from lon/lat to a local metric CRS.
    x_dme, y_dme = to_los_crs.transform(lon_dme, lat_dme)
    x_aircraft, y_aircraft = to_los_crs.transform(lon_aircraft, lat_aircraft)

    # Start and end altitude of the radio ray.
    z_start = alt_dme + dme_antenna_height_m
    z_end = alt_aircraft + aircraft_antenna_height_m

    # Build the 3-D ray in x/y/z.
    x_ray = x_dme + fractions * (x_aircraft - x_dme)
    y_ray = y_dme + fractions * (y_aircraft - y_dme)
    z_ray = z_start + fractions * (z_end - z_start)

    # Convert the sampled x/y positions back to lon/lat for raster lookup.
    # The z coordinate remains the ray altitude computed above.
    lons, lats = from_los_crs.transform(x_ray, y_ray)
    ...
    if not np.all(valid):
        # Missing/outside terrain samples are ignored, not treated as blockers.
    ...
    z_ray = z_ray[terrain_valid]

    if len(terrain_vals) < 3:
        return True

    # Ignore first and last point: the DME and aircraft locations themselves.
    terrain_between = terrain_vals[1:-1]
    ray_between = z_ray[1:-1]

    # Visible if the terrain is always below the 3-D radio ray.
...
for iy in range(ny):
    ...
    for ix in range(nx):
        ...
        vertical_dist_km = (aircraft_altitude_m - dme_alts) / 1000
        
        slant_dist_km = np.sqrt(horizontal_dist_km**2 + vertical_dist_km**2)
        
        candidates = np.where(slant_dist_km <= dme_ranges)[0]
